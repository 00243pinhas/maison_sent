    ...
    BullModule.forRootAsync({
      ...
      useFactory: (config: ConfigService) => ({
        connection: {
          ...
          // Stop retrying after 1 attempt so logs stay clean when Redis is absent
          retryStrategy: (times: number) => (times <= 1 ? 500 : null),
        },
...
export class JobsModule implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        'Redis is unreachable — background jobs are disabled. Start Redis to enable queues.',
      );
      return;
    }
